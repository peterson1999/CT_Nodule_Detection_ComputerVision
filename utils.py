...
# -*- coding: utf-8 -*-
"""
Created on Mon Jul 22 15:04:16 2024
...
IRCTuple = collections.namedtuple('ImageCoordinates', ['index','row','column'])
XYZTuple = collections.namedtuple('PatientCoordinates',['x','y','z'])

def IRCtoXYZ(irc_coordinates,origin_xyz,voxel_size,direction):
    cri_coordinates = np.array(irc_coordinates)[::-1]
    origin_array = np.array(origin_xyz)
    voxel_size_array = np.array(voxel_size)
    xyz_coordinates = ((cri_coordinates*voxel_size_array) @ direction)+origin_array
    return(XYZTuple(*xyz_coordinates))

def XYZtoIRC(xyz_coordinates, origin_xyz, voxel_size,direction):
    origin_array = np.array(origin_xyz)
    voxel_size_array = np.array(voxel_size)
    irc_coordinates = ((np.array(xyz_coordinates)-origin_array)@ np.linalg.inv(direction))/voxel_size
    irc_coordinates = np.round(irc_coordinates)
    return(IRCTuple(int(irc_coordinates[2]),int(irc_coordinates[1]),int(irc_coordinates[0])))
